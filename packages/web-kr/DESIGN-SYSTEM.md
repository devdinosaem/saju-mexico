...
1. **가독성 중요하면 흰색, 그 외엔 파스텔 적극** — 본문·긴 글·리포트처럼 **읽는 카드**는 흰색(`--surface-card`, 가독성 우선). 그 외 **짧은 라벨·카테고리 타일·네비 버튼·강조 박스(InfoBox)·그라디언트(Hero)** 엔 파스텔을 적극 쓴다(예: 운테리어 소품/캐릭터/스킨 파스텔 버튼 👍). 페이지 바탕은 크림(`--surface-page`). ※아이보리 #FFFCF6은 본문 카드엔 노란기로 부적합 — 읽는 면엔 쓰지 말 것.
...
4. **폰트 역할 고정 + 가독성 우선** — 본문/서브텍스트 주(主)는 **Pretendard**, 제목은 **빙그레**, 감성 포인트는 **온글잎 박다현체**(짧은 어절·단일두께라 bold 금지). **gaegu(Cafe24동동)는 사주카드·셀러브리티 카드·사주상품 탭에서만, 그 외 절대 금지** (아래 §3).
5. **일주 프로필·두들 풍부히** — 일주 프로필 아바타(그라데이션 링+오행 배경+두들 얼굴)는 우리 자산, 그대로 계승. '나' 관련 카드엔 프로필을, 섹션·박스엔 두들 스티커를 **가급적 풍부히** 넣어 생기를 준다.
...
| `--surface-card` | `#FFFFFF` | **기본 카드** (흰색·가독성) |
...
| 역할 | 폰트 | 토큰 / 유틸 | 쓰는 곳 |
|---|---|---|---|
| **제목·강조·브랜드** | BinggraeTaom (빙그레타옴) | `FONT.title` / `.font-title` | 섹션 제목, 카드 타이틀, 큰 숫자/브랜드 |
| **기능·데이터·본문(主)** | Pretendard(기본) | 지정 불필요 | **모든 설명·서브텍스트·긴 본문**, 라벨, 표/수치, 일주명 |
| **감성 포인트(점)** | **온글잎 박다현체** (Ownglyph ParkDaHyun) | `FONT.flavor` / `.font-flavor` | **짧은 어절·한 줄 카피·`"인용"` 에만** |

> **감성 포인트 폰트 = 온글잎 박다현체로 결정** (기존 Cafe24동동=gaegu 대체). 후보 비교 페이지: [`/design/fonts`](http://localhost:3001/design/fonts).
> **⚠️ 단일 두께(Regular)뿐** → `font-bold`/`<strong>` 금지 (faux-bold로 손글씨가 뭉개짐). 강조는 **색(핑크)·제목폰트(빙그레)** 로 분리.
> **가독성 규칙(중요).** 본문·서브텍스트 기본은 **Pretendard**, 제목은 **빙그레**. 감성체는 **문장 전체에 깔지 말 것** — 짧은 태그라인/한 줄 멘트/인용 등 **포인트로만**.
> **적용 현황(2026-06-29).** `@font-face` 로딩 완료(`globals.css`). 현재 **self v2 리포트에서만 시범 적용**(로컬 `OWNGLYPH` 상수). 전역 토큰 `--font-flavor`는 아직 Cafe24동동 — **전역 전환 시 `--font-flavor` 한 줄 교체**(+CDN 의존 제거 위해 self-host 권장).

> **🚫 Cafe24동동(gaegu) 사용 범위 — 엄격 제한.** gaegu는 **오직 ①사주카드 ②셀러브리티 카드 ③사주상품 탭(1탭)** 에서만 허용한다(브랜드 정체성 보존). **그 외 모든 화면·컴포넌트에서 절대 금지.** 기존에 gaegu(`FONT.flavor`/`GAEGU`/`Cafe24Dongdong`)로 쓰인 곳은 폰트 역할에 맞춰 교체한다 — **본문·데이터·서브텍스트 → Pretendard, 짧은 감성 포인트 → 온글잎 박다현체.** (예: 리포트의 `SinsalCard`·`SinsalBadge` 등 잔존 gaegu = 전환 대상.) ※ 허용된 3곳도 gaegu가 *필수*는 아님 — 박다현/Pretendard로 가도 무방, 단 gaegu가 **나올 수 있는 곳**은 이 3곳뿐.
...
| `<Card raised flat>` | 표준 흰 카드(읽는 면). `raised`=진한 테두리 |
| `<Section icon title basis card>` | 섹션 묶음(제목+본문). `card`면 본문을 카드로 감쌈 — 반복 래퍼 제거 |
...
도메인(사주) 전용 — `Avatar`/`EmptySlot`/`ElemBar`/`SplitBar`/`ElementSticker`/`ElementBadge`는 [`lib/saju-play/ui.tsx`](src/lib/saju-play/ui.tsx).

### 레이아웃·장식 패턴
- **아이콘-리드 카드**: 두들/프로필이 좌측에서 타이틀+서브를 이끈다 — `[아이콘][제목 / 서브(우측 정렬)]`. InfoBox·"올해 흐름"·"본캐 vs 부캐"가 이 형태. 두들을 타이틀 줄에 두고 서브는 타이틀에 맞춰 들여쓴다(서브가 아이콘 아래로 내려가지 않게).
- **프로필 카드**: '나' 정체성 카드(스펙시트·연애 스타일·에너지 운용·사용설명서 등)는 좌측에 일주 `Avatar`(40~52px)를 둔다.
- **두들·프로필 적극**: 빈 섹션/박스엔 의미에 맞는 두들을, 정체성 카드엔 프로필을 풍부히 (원칙 §5).
...
- [x] Phase 0 — 토큰 + 키트(+그라디언트·오행 컴포넌트) + 문서 + `/design`·`/design/fonts` 쇼케이스
- [~] **Phase 1 — 최종리포트 v2** (`self/report.v2.tsx`, `SelfReportV2`) — *승인 대기*
  - v1↔v2 비교: [`/v3/self/compare`](http://localhost:3001/v3/self/compare) (v1은 보존)
  - 적용: 키트 재구성 · 색 박스→InfoBox 통합 · depth 평탄화(`Section`) · 흰 카드 · 섀도우 제거 · 그라디언트 Hero · 폰트(박다현/Pretendard) · 프로필·두들 보강 · `deriveSelf`로 v1/v2 로직 공유
  - **승인 후 할 일**: ① 펀널([`self/core.tsx`])·보관함([`report/[id]`]) import를 v2로 교체 + v1 제거 ② 감성 폰트 전역화(`--font-flavor`→박다현 + self-host)
- [ ] Phase 2 — 상세 펀널(sinsal/nextmonth/some/onesided) — 표지/페이월/로딩 공용화
- [ ] Phase 3 — shop 탭 + 하단 네비(글리프→두들) + 안티패턴 청산(`box-shadow`·`border-2 border-charcoal`·인라인 hex·이모지, ~91곳)
