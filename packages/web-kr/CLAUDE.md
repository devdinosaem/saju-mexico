...
- [ ] **이번 주 에너지 흐름** (구 "주간 에너지 그래프")
  월~일 7일 운세 → 이번 주 베스트/워스트 날
  출력: 7일 에너지 바 그래프 + 오늘 강조 + "수요일이 베스트!"
  위치: **5탭 마이 최하단(광고 배너 바로 위)** — 3탭 운기달력에서 이전됨
  데이터: saju-engine 7일 일진 → 유저 일주 대비 에너지 점수 (현재 목업)
  컴포넌트: my/_components/WeeklyEnergyFlow.tsx
  진행: 이전·개명·7일 영역 골격 완료 / 오늘강조·베스트워스트·실데이터 연동 대기
  수익: 구독 포함

- [ ] **오행 밸런스 트래커** 🗄️ 백로그 (구현체 FoodLogger 제거됨)
  매일 1탭 오행 체크인 (5초) → 주간 오행 분포 집계 → 용신 기반 맞춤 조언
  출력: 🌳1 🔥3 ⛰️1 💎0 💧2 → "화(火) 과잉, 수(水) 활동 추천"
  수익: 주간 리포트는 구독자만
  상세 기획·UI·구현 정책: backlog/components/v3/calendar/FoodLogger.POLICY.md
