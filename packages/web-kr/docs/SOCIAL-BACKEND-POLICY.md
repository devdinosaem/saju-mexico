...
초대 링크(`/v3/add/<friend_code>`)는 **stateless** — "이 코드 주인을 친구추가"만 담고, **"누가 누구를 초대했는지"는 기록하지 않는다.** 따라서:

| 케이스 | 식별 | 처리 |
|---|---|---|
| 초대 링크를 탭 + 로그인 | ✅ | 즉시 요청→수락 (구현됨) |
| 링크 탭했으나 미가입 | ⚠️ 부분 | (선택) localStorage에 코드 저장 → *같은 브라우저* 가입 시 자동 요청. 기기/경로 바뀌면 불가 |
| **다른 경로로 가입한 친구** | ❌ 링크로는 불가 | **카카오 친구 동기화로 해결** — 링크가 아니라 *공통 소셜 그래프*로 식별. 인증(Phase 2) 후 과제 |

→ **결정**: 링크는 "탭→즉시 추가" 경로만 책임진다. "내 링크 안 쓰고 딴 데로 가입한 친구" 식별은 **카카오 친구 동기화(post-auth)**의 몫. 연락처 매칭은 후순위.

## 13. 친구 요청 다건 표시 정책

`FriendRequests`(운테리어 상단)는 단일 카드 + 세로 리스트. 요청이 많아지면 카드가 길어져 하위 콘텐츠를 밀어냄.
→ **정책(검토 중)**: 상위 N개만 노출 + `+N개 더보기`(펼치기 또는 전용 화면). 현재는 전체 리스트(다건 더미로 디자인 평가 중).
