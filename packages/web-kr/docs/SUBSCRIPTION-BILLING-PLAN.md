...
- ✅ **프리미엄 캘린더**: 길일/택일 **표시** + 중요일 **표시**(구독자만). **알림(푸시)은 미구현** — 캘린더 마킹·요약만. `MonthCalendar`에 `getDayFortune` overall(≥4 길일·≤2 주의일) 마킹 + 이달 요약 + 비구독 티저.
- ✅ **① 무료 리셋 기준 = 캘린더월**(매월 1일 리셋, `benefitMonthKey`).

> 추후(P5+): 중요일 **푸시 알림**은 Capacitor 네이티브 단계에서. 시간대별 운(시진)·일진 상세는 확장 여지.
