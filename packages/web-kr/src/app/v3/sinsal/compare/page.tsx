"use client"
// ════════════════════════════════════════════════════════════════
// 내 신살 도감 — v1 ↔ v2 비교 (디자인 시스템 적용 전/후). 페이월·연출 없음.
// ════════════════════════════════════════════════════════════════
import { useState } from "react"
import { useUser } from "@/lib/UserContext"
import { buildSinsal, type SinsalBirth, type Gender } from "@/lib/saju-play/sinsal/sinsal-adapter"
import { to24h } from "@/lib/saju-play/crush/saju-adapter"
import { SinsalReport, sinsalFallback } from "@/lib/saju-play/sinsal/report"
import { SinsalReportV2 } from "@/lib/saju-play/sinsal/report.v2"
import { FONT } from "@/lib/ds"

const FALLBACK_BIRTH: SinsalBirth = { year: 1995, month: 3, day: 15, hour: 12, minute: 0 }

export default function SinsalComparePage() {
  const { user } = useUser()
  ...
  const aiText = sinsalFallback(data)
    ...
    <div className="flex flex-col gap-4 pb-6">
      <div className="pt-1">
        ...
        <p className="text-[12px] text-text-muted text-center mt-1.5" style={FONT.flavor}>
          {v === "v1" ? "현재 라이브 화면" : "디자인 시스템 적용 — 흰 카드·소프트·그라디언트 Hero(오행 테두리)·InfoBox·박다현/Pretendard"}
        </p>
      </div>

      {v === "v1"
        ? <SinsalReport data={data} aiText={aiText} />
        : <SinsalReportV2 data={data} aiText={aiText} />}
    </div>
  )
}
