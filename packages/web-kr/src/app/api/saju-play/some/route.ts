...
- 궁합 데이터의 '지금 연애 흐름'이 잔잔(타이밍 안 뜨거움)으로 나와도 "넌 연애운이 멀었어"로 절망 주지 마 — "지금은 흐름이 잔잔할 뿐, 인연은 네가 만드는 거고 가까운 흐름은 이렇게 탄다"로 가까운 행동·계기로 돌려.
...
마지막: 짧고 따뜻한 응원 한 줄.`
  ...
  const text = withHook(hookSome(them.name), data.choices?.[0]?.message?.content?.trim() ?? "")
