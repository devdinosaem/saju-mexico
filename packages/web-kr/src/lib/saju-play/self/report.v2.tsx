...
import {
  Ico, Card, InfoBox, Section, SectionTitle, ChapterDivider, Hero, Prose, GradBadge,
} from "@/components/ds"
import {
  ...
  return (
    <div className="rounded-[var(--r-lg)] px-4 py-3.5 flex items-start gap-2.5"
      ...
      <div className="min-w-0 flex-1">
        ...
        {children && <div className="text-[14px] text-charcoal/75 leading-snug mt-0.5" style={FONT.flavor}>{children}</div>}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <>
            ...
            {x.iljuType.weaknesses?.length > 0 && (
              <div className="flex flex-col gap-1.5">
                ...
                <p className="text-[13px] text-charcoal/55 leading-snug" style={FONT.flavor}>이런 면도 있는데, 알아두면 오히려 다루기 쉬워.</p>
              </div>
            )}
          </>
        )}
      ...
      <Section icon={DoodleCrown} title="사주가 같은 사람" basis="일주">
        {x.celeb && x.celeb.persons.length > 0 ? (() => {
            ...
            <div className="flex justify-center items-start gap-2 pt-2 pb-1">
              {cs.length >= 2 ? (
                <>
                  ...
                  <NaCard iljuKey={x.charKey} role={naRole} elem={self.dayElem} idx={1} />
                  <CelebCard name={cs[1].name} role={cs[1].role} cat={cs[1].cat} elem={self.dayElem} idx={2} />
                </>
              ) : (
                <>
                  <NaCard iljuKey={x.charKey} role={naRole} elem={self.dayElem} idx={0} />
          ...
          <Card className="px-4 py-4 text-center">
            <p className="text-[14px] text-charcoal/60 leading-snug" style={FONT.flavor}>{x.bareIlju}일주는 아직 등록된 유명인이 없어 — 네가 1호일지도?</p>
          </Card>
        )}
        <p className="text-[13px] text-text-muted text-center" style={FONT.flavor}>너도 이 기운을 타고났어.</p>
      </Section>
      ...
      <Section icon={DoodleTaegeuk} title="내 오행 밸런스" basis="오행 분포" card bodyClass="flex flex-col gap-2.5">
        ...
        <p className="text-[14px] text-charcoal/70 leading-snug pt-1" style={FONT.flavor}>
          나를 살리는 기운은 <span className="font-bold" style={{ color: "var(--pink)" }}>{self.yong}</span> · 조심할 기운은 {self.gi}.
        </p>
      </Section>
      ...
      <Section icon={DoodleLightning} title="에너지 운용" basis="신강신약" card bodyClass="flex flex-col gap-2.5">
        ...
        <div className="relative h-3 rounded-full" style={{ background: "linear-gradient(90deg,#93C5FD,#E5E7EB,#F9A8C4)" }}>
          <span className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-5 h-5 rounded-full bg-white shadow-sm" style={{ left: `${x.energyPos}%`, border: "2px solid var(--pink)" }} />
        </div>
        <p className="text-[14px] font-bold text-charcoal">{self.strongLevel}</p>
        <p className="text-[14px] text-charcoal/70 leading-snug" style={FONT.flavor}>{self.isStrong ? "에너지 풀충전형 — 내가 키 쥐고 밀어붙일 때 잘 풀려. 가끔 브레이크만 잊지 마." : self.strongLevel.includes("중화") ? "균형형 — 상황 따라 유연하게 가는 게 강점이야." : "섬세·기댐형 — 좋은 환경·사람에 기대고 채울 때 빛나. 무리한 독주는 방전돼."}</p>
      </Section>
          ...
          {self.topTalent.map((gp, i) => {
            ...
            return (
              <Card key={i} className="px-3 py-3.5 flex flex-col gap-1.5">
                ...
                <p className="text-[13px] text-charcoal/65 leading-snug" style={FONT.flavor}>{t.line}</p>
              </Card>
            )
          })}
        </div>
      ...
      {/* 본캐 vs 부캐 */}
      <Section icon={DoodleMirror} title="본캐 vs 부캐" basis="일간·오행" card bodyClass="flex flex-col gap-2.5">
        {x.sameCore ? (
          <p className="text-[14px] text-charcoal/75 leading-snug" style={FONT.flavor}>겉과 속이 일관된 사람. 보이는 그대로 — <span className="font-bold text-charcoal">{ELEM_TRAIT[x.ego]}</span>.</p>
        ) : (
          <>
            <div className="flex items-start gap-2.5"><span className="text-[13px] font-bold w-12 shrink-0" style={{ color: "var(--pink)" }}>겉(부캐)</span><span className="text-[14px] text-charcoal/75 leading-snug" style={FONT.flavor}>{ELEM_TRAIT[x.persona]}</span></div>
            <div className="flex items-start gap-2.5"><span className="text-[13px] font-bold w-12 shrink-0" style={{ color: "var(--pink)" }}>속(본캐)</span><span className="text-[14px] text-charcoal/75 leading-snug" style={FONT.flavor}>{ELEM_TRAIT[x.ego]}</span></div>
          </>
        )}
      </Section>
            ...
            return (
              <div key={e} className="flex items-center gap-2.5 py-2.5 border-b border-charcoal/5 last:border-0" style={n === 0 ? { opacity: 0.4 } : undefined}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[13px] text-charcoal/60 leading-snug" style={FONT.flavor}>&ldquo;{m.voice}&rdquo;</p>
                </div>
              </div>
            )
          })}
        </Card>
        <p className="text-[13px] text-text-muted leading-snug" style={FONT.flavor}>제일 목소리 큰 {x.meetingOrder[0]}({MEETING[x.meetingOrder[0]].role})이 너를 주로 끌고 가. {x.weakElems.length ? `빠진 ${x.weakElems.join("·")}은 의식적으로 챙겨야 균형이 맞아.` : ``}</p>
      </Section>
      ...
      <Section icon={DoodleHeart} title="내 연애 스타일" basis="일간·도화" card bodyClass="flex flex-col gap-1.5">
        ...
        <p className="text-[14px] text-charcoal/70 leading-snug" style={FONT.flavor}>{LOVE_STYLE[self.dayElem].line}.{self.dohwa ? " 매력(도화) 기운이 있어 끌어당기는 힘도 있어." : ""}</p>
      </Section>
      ...
      <div className="grid grid-cols-2 gap-2">
        <Link href="/v3/some" className="ds-infobox ds-accent-love px-3 py-3 flex flex-col gap-1 active:opacity-85 transition-opacity">
          ...
          <p className="text-[12px] text-charcoal/55 leading-snug" style={FONT.flavor}>그 사람과 나, 가능성은?</p>
        </Link>
        <Link href="/v3/onesided" className="ds-infobox ds-accent-special px-3 py-3 flex flex-col gap-1 active:opacity-85 transition-opacity">
          ...
          <p className="text-[12px] text-charcoal/55 leading-snug" style={FONT.flavor}>그 사람, 내 맘 알까?</p>
        </Link>
      </div>
      ...
      <Section icon={DoodleMedal} title="어울리는 일" basis="십신" card bodyClass="flex flex-col gap-2">
        {self.topTalent.map((gp, i) => (
          <div key={i} className="flex items-start gap-2.5">
            ...
            <p className="text-[14px] text-charcoal/75 leading-snug" style={FONT.flavor}>{JOB[gp]}</p>
          </div>
        ))}
      </Section>
      ...
      <Section icon={DoodleDiamond} title="재물 그릇" basis="재성·대운" card bodyClass="flex flex-col gap-2.5">
        <p className="text-[14px] text-charcoal/75 leading-snug" style={FONT.flavor}>{x.moneyLine}</p>
        <div className="ds-infobox ds-accent-warn px-3 py-2.5 flex items-center gap-2">
          <Ico as={DoodleCalendar} size={16} />
          <p className="text-[13px] text-charcoal/75 leading-snug" style={FONT.flavor}>{x.moneyAges.length ? `돈이 트이는 시기: ${x.moneyAges.slice(0, 3).join(", ")} 전후` : "한 방보다 쌓는 게 맞아 — 꾸준함이 돈으로 와."}</p>
        </div>
      </Section>
      ...
      <Section icon={DoodleStar} title="내 인생 그래프" basis="대운" card bodyClass="flex flex-col gap-2">
        ...
        <p className="text-[14px] text-charcoal/70 leading-snug text-center" style={FONT.flavor}>{self.peak ? `황금기는 ${self.peak.ageLabel}세 전후. ` : ""}지금({self.curAge}세)은 {x.curFavorLine}이야.</p>
      </Section>
        ...
        <Card className="overflow-hidden">
          {([["취급주의", x.manual.care], ["충전법", x.manual.charge], ["방전 신호", x.manual.drain], ["셀프 A/S", x.manual.as]] as const).map(([k, v]) => (
            <div key={k} className="px-4 py-2.5 flex gap-3 border-b border-charcoal/5 last:border-0">
              ...
              <span className="text-[14px] text-charcoal/75 leading-snug" style={FONT.flavor}>{v}</span>
            </div>
          ))}
        </Card>
      </Section>
      ...
      <Section icon={DoodleCalendar} title="올해 흐름" basis="세운" card bodyClass="flex flex-col gap-2">
        ...
        <p className="text-[14px] text-charcoal/70 leading-snug" style={FONT.flavor}>무리해서 판을 뒤집기보다, 이 흐름에 맞춰 한 발씩 가면 돼.</p>
      </Section>
      ...
      <Section icon={DoodleStar} title="사주 RPG 스탯" basis="오행·십신" card bodyClass="flex flex-col gap-1">
        <Radar data={x.stats} />
        <p className="text-[14px] text-charcoal/70 leading-snug text-center" style={FONT.flavor}>최고 스탯은 <GradBadge theme={self.dayElem} className="!px-2 !py-0.5 text-[13px] align-middle">{x.topStat.label} {x.topStat.value}</GradBadge> — 여기로 승부 봐.</p>
      </Section>
      ...
      <Link href="/v3/consult" className="rounded-[var(--r-xl)] px-4 py-4 flex items-center gap-3 active:opacity-90 transition-opacity"
        style={{ background: "var(--grad-pink-surface)", border: "1px solid var(--line-soft)", boxShadow: "var(--shadow-md)" }}>
        <Ico as={DoodleSpeechBubble} size={24} />
        <div className="flex-1 min-w-0">
          <p className="text-[15px] text-charcoal" style={FONT.title}>더 궁금한 건 대화로 물어봐</p>
          <p className="text-[13px] text-charcoal/60 leading-snug" style={FONT.flavor}>내 사주 캐릭터한테 직접 — 연애·일·고민 뭐든</p>
        </div>
